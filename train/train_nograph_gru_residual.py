...
from models.baseline_no_graph_gru import NoGraphGRU
...
def train_one_epoch(
    ...
    """
    单个 epoch 的训练（残差模式）：
    - naive_y：复制过去 T_out 个 main_power（标准化）
    ...
    for X, y in loader:
        ...
        # naive baseline：复制过去 T_OUT 的 main_power（标准化空间）
        main_seq = X[:, :, main_idx, :]       # (B, T_in, F_max)
        naive_y = main_seq[:, -t_out:, 0]     # (B, T_out)
...
def eval_one_epoch(
    ...
    # 真实空间 RMSE / MAPE 累计（和 Graph-GRU 同一口径）
    total_se_kw = 0.0
    total_ape = 0.0
    n_points = 0
    ...
    for X, y in loader:
        ...
        main_seq = X[:, :, main_idx, :]
        naive_y = main_seq[:, -t_out:, 0]        # baseline（标准化空间）
...
def main():
    """
    主函数：残差版 NoGraph-GRU 训练（超参与 Graph-GRU 方案 A 对齐）。
    """
    # ====== 超参数（与 Graph-GRU 对齐） ======
    T_IN = 288      # 过去 24h
    T_OUT = 288     # 预测 24h
    ...
    train_history = []       # train 残差 loss
    val_history = []         # val 残差 loss
    val_rmse_history = []    # val RMSE (kW)
    val_mape_history = []    # val MAPE (0~1)
    ...
    if best_state_dict is not None:
        ...
        torch.save(
            ...
            "nograph_gru_residual_best.pt",
        )
        print(
            f"Saved best residual NoGraph-GRU model to nograph_gru_residual_best.pt, "
